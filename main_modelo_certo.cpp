...
void stepGazebo(ros::ServiceClient& step_physics_client, std_srvs::Empty& srv) {
  ...
  // Modifica os elementos da diagonal da matriz Qh com os valores fornecidos
    ...
    void config()
    {
        ...
        // SX v_i = SX::vertcat({SX(0), SX(5), SX(6), SX(7), SX(0), SX(0), SX(0), SX(0)});
        SX v_i = SX::zeros(8, 1);
        J=0;

        std::ifstream file("/home/pedro/catkin_ws/src/ProVANT-Simulator_Developer/source/Structure/control_strategies/quadrotor_mpc/src/Inputs_gazebo.txt");
        Eigen::MatrixXd M(4, 200); // matriz 4x200

        if (!file.is_open()) {
            std::cerr << "Erro ao abrir o arquivo Inputs_gazebo.txt!" << std::endl;
        } else {
            for (int i = 0; i < 4; ++i) {
                for (int j = 0; j < 200; ++j) {
                    file >> M(i, j);
            ...
            std::cout << "Matriz carregada com sucesso!" << std::endl;
            std::cout << M.block(0, 0, 4, 5) << std::endl; // mostra as 5 primeiras colunas
        }
        SX estados_t = SX::vertcat({SX(1), SX::zeros(15,1)});
        // SX estados_t = SX::vertcat({0.15425, 0, 0, -0.98803, 1.9761, -1.3278, 1.2194, 0.3085, 0, 0, 0, 0, 0, 5, 6, 7}); 
        Eigen::MatrixXd saidateste = Eigen::MatrixXd::Zero(16, 200);
    ...
    for (int i = 0; i < saidateste.rows(); ++i) {
        for (int j = 0; j < saidateste.cols(); ++j) {
            saida_file << saidateste(i, j);
            if (j < saidateste.cols() - 1)
                saida_file << " ";
        }
        saida_file << "\n";
    }
    ...
    throw std::runtime_error("Terminoudo a execução após salvar a matriz.");
        ...
        DM ome_init = DM::vertcat({0, 0.01, 0.01, 0.01});
    ...
    0.0, 0.0, 0.0, 0.0                      // Velocidade linear desejada
        ...
        std::cout << "Espere2\n"<< std::endl;
        // if (verif == 1 ) {
        //     std::cout << "Função objetivo J: " << J << std::endl;
        // }
        // verif = 1;
    }
 ...
 // Get the current simulation time and calculate the reference trajectory.
    ...
    // std::cout << "Posicao: " << prox_pos << std::endl; //pode excluir
